...
class Trace
{
    public:
        ...
        void Aqcuire_Sweep_Buffered(int frameCounter, const std::string& folderName, int FFTLength, int16_t* StreamBuffer, int bufferSize) 
        {
            // 创建文件路径并打开文件
            std::ostringstream fileName;
            fileName << folderName << "/sdr_data_frame_" << frameCounter << ".txt";
            ...
            for (int z = 0; z < (int)ceil(Display.span / Display.resBW); z++) {
                ...
                // 将 StreamBuffer 中的数据直接写入文件，每对 I/Q 数据一行
                for (int i = 0; i < samplesRead*2; i++) {
                    outFile << StreamBuffer[i] << " ";  // I 和 Q 各一列
                }
            }
...
int main(int argc, char** argv)
{
    ...
    float_type span = 30e6;          // Frequency span
    ...
    std::string folderName = "sdr_data_frames_txt";
