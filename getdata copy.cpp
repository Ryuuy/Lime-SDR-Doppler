...
class Lime_SDR_mini_Rx
{
    ...
    public:
        ...
        int Setup_SDR(float_type centerFreq, float_type sampleRate, unsigned gain)
        {
            ...
            const char *config_file = "./2386.ini";
...
void Aqcuire_Sweep_Buffered(int frameCounter, const std::string& folderName, int FFTLength, int16_t* StreamBuffer, int bufferSize, int Repeatnumber) 
{
    for (int z = 0; z < (int)ceil(Display.span / Display.resBW); z++) {
        // 设置频率
        double currentFreq = Display.centerFreq + 10e6 * z;//SDRSetting.sampleRate
        Change_RF_Settings(currentFreq);
        ...
        for (int i = 0; i < Repeatnumber; i++) {
            ...
            if (samplesRead > 0) {
                ...
                std::cerr << "Failed to receive samples for freq " << z << ", repeat " << i << std::endl;
            }
        }
...
void Aqcuire_Sweep_Buffered_withdelay(int frameCounter, const std::string& folderName, int FFTLength, int16_t* StreamBuffer, int bufferSize, int Repeatnumber, int delay_ms) 
{
    using namespace std::chrono;

    // 动态修改文件夹名称，添加 "withdelay...ms" 后缀
    std::ostringstream folderNameWithDelay;
    folderNameWithDelay << folderName << "_withdelay" << delay_ms << "ms";

    // 确保新目录存在
    createDirectory(folderNameWithDelay.str());

    for (int z = 0; z < (int)ceil(Display.span / Display.resBW); z++) {
        // 设置频率
        double currentFreq = Display.centerFreq + 10e6 * z;
        Change_RF_Settings(currentFreq);
        ...
        for (int i = 0; i < Repeatnumber; i++) {
            ...
            if (samplesRead > 0) {
                // 创建文件路径
                std::ostringstream fileName;
                fileName << folderNameWithDelay.str()  // 使用添加延迟后缀的新目录
                         << "/center" << static_cast<int>(currentFreq / 1e6) 
                         << "frame" << frameCounter 
                         << ".bin";
            ...
            // 如果接收数据的时间小于延迟时间，进行额外延迟
            if (elapsed_ms < delay_ms) {
                std::this_thread::sleep_for(milliseconds(delay_ms - elapsed_ms));
            } else {
                // 如果接收数据的时间超过了延迟时间，打印超时警告
                std::cerr << "Warning: Processing time (" << elapsed_ms 
                          << " ms) exceeded delay (" << delay_ms << " ms)." << std::endl;
            }
        }
        std::cout << "Next saved frame: " << frameCounter << ", freq index: " << z << std::endl;
    }
}
...
int main(int argc, char** argv)
{
    ...
    for (int u = 0; u < 1; u++) {
        ...
        Trace1.Aqcuire_Sweep_Buffered(framenumber, folderName, FFTlength, StreamBuffer, bufferSize, repeat);  // Pass the required parameters
        //Trace1.Aqcuire_Sweep_Buffered_withdelay(framenumber, folderName, FFTlength, StreamBuffer, bufferSize, repeat, delay_ms);
