...
class Lime_SDR_mini_Rx
{
    ...
    public:
        ...
        int Setup_SDR(float_type centerFreq, float_type sampleRate, unsigned gain)
        {
            ...
            // LMS_Calibrate(device, LMS_CH_RX, 0, sampleRate/2, 0);
...
class Trace
{
    public:
        Trace(float_type centerFreqIn, float_type dispSpanIn,  float_type resBWIn, double gainIn, double refAmpIn, int FFTIn, int averaging, unsigned TraceNum):FFT(8192)
        {
            ...
            YAxisBuffer = new double [int(ceil(FFTIn*(Display.span/Display.resBW)))];
            // std::cout << "YAxisBuffer size: " << int(ceil(FFTIn * (Display.span / Display.resBW))) << std::endl;

            YAxisBufferAvg = new double [int(ceil(FFTIn*(Display.span/Display.resBW)))*(averaging+1)];
            XAxisBuffer = new double [int(ceil(FFTIn*(Display.span/Display.resBW)))];
            //printf("Buffer size calculated: %d\n", int(ceil(FFTIn / 2 * (Display.span / Display.resBW))));
            for(int x=0; x<(int)(ceil(FFTIn*(Display.span/Display.resBW))); x++)
                YAxisBuffer[x] = 0;
            // The averaging buffer will be <averaging> times bigger than the rest
            for(int x=0; x<(int)(ceil(FFTIn*(Display.span/Display.resBW))*averaging); x++)
                YAxisBufferAvg[x] = 0;
            for(int x=0; x<int(ceil(FFTIn*(Display.span/Display.resBW))); x++)
                XAxisBuffer[x] = SDRSetting.centerFreq - SDRSetting.sampleRate / 2 + (SDRSetting.sampleRate / FFT.DFTlength)*x;
            // Stream will fit the data as follows I,Q,I,Q,I,Q... --> double the size of DFTlength
            StreamBuffer = new int16_t[FFT.DFTlength * 2];
            
            // Setup SDR
            // std::cout << "Center Frequency: " << SDRSetting.centerFreq << " Hz" << std::endl;
            // std::cout << "Sample Rate: " << SDRSetting.sampleRate << " Hz" << std::endl;
            // std::cout << "Gain: " << SDRSetting.gain << std::endl;
        ...
        void Aqcuire_Sweep()
        {
            ...
            int sweepBins = (int)ceil(Display.span/Display.resBW*FFTLength);
            // std::ofstream outFile("stream_buffer_data.txt");
            ...
            for(int z = 0; z<(int)ceil(Display.span/Display.resBW); z++)
            {
                ...
                std::cout << "Result (SDRSetting.centerFreq): " 
                    << (SDRSetting.centerFreq) << std::endl;
                ...
                //     outFile << StreamBuffer[i] << " ";  // 将数据以空格分隔，或使用 "\n" 换行符进行分行
                // }
                // Fill FFT.in --> I data goes into FFT.in[x][0], Q data into FFT.in[x][1]
                for(int x=0; x<samplesRead*2; x+=2)
                {
                ...
                for (int y = 0; y < FFTLength / 2; y++) {
                    int swapIndex = y + FFTLength / 2;

                    // 交换实部
                    double tempReal = FFT.out[y][0];
                    FFT.out[y][0] = FFT.out[swapIndex][0];
                    FFT.out[swapIndex][0] = tempReal;

                    // 交换虚部
                    double tempImag = FFT.out[y][1];
                    FFT.out[y][1] = FFT.out[swapIndex][1];
                    FFT.out[swapIndex][1] = tempImag;
                }
                // If averaging is not required
                if(Display.averagingCntr == -1)
                {
                    //Data in Yindex
                    for(int y=0; y<FFTLength; y++)
                    {   // 0.000195313 V equals one step of ADC (only valid when stream data is configured as 12-Bit integers!!)
                        ...
                        YAxisBuffer[z*FFTLength+y] = 10*log10(pow(0.000195313 * pow(FFT.out[y][0]*FFT.out[y][0]+FFT.out[y][1]*FFT.out[y][1], 0.5)/FFTLength, 2)/50/0.001);
                        //XAxisBuffer[z*FFTLength+y] = SDRSetting.centerFreq + (SDRSetting.sampleRate / FFTLength)*y;
                        
                    }
                                    
                    std::cout << "Ybufferindex" 
                    << (z*FFTLength) << std::endl;
                }
                // Averaging required
                else
                {
                    ...
                    for(int y=0; y<FFTLength; y++)
                    {
                        ...
                        YAxisBufferAvg[AveragingCounter*sweepBins+(z*FFTLength+y)] = pow(0.000195313 * pow(FFT.out[y][0]*FFT.out[y][0]+FFT.out[y][1]*FFT.out[y][1], 0.5)/FFTLength, 2)/50;

                        if(initAveraging == false)
                        {
                            double averagedPower = 0;
                            // Add up power for this bin from current + previous sweeps
                            int je = sweepBins*AveragingCounter+1+(z*FFTLength+y); //DEBUG
                            for(int x=0; x<AveragingCounter+1; x++)
                                averagedPower += YAxisBufferAvg[sweepBins*x+(z*FFTLength+y)];

                            // Divide by number of previous + current sweeps stored
                            averagedPower /= AveragingCounter+1;

                            // convert to dBm
                            YAxisBuffer[z*FFTLength+y] = 10*log10(averagedPower/0.001);

                            // Remove gain
                            YAxisBuffer[z*FFTLength+y] = YAxisBuffer[z*FFTLength+y] - SDRSetting.gain;
                        }
                        else
                        {
                            double averagedPower = 0;
                ...
                // Plot the spectrum warning problem
        ...
        void Plot_Spectrum()
        {
            ...
            for(int x=0; x<(int)ceil(Display.span/Display.resBW)*FFTLength; x++)
            {
    ...
    private:
        ...
        void CH_Measurement()
        {
            ...
            int indexStartFreq = int(ceil((int(ceil(getFFTLength()*(Display.span/Display.resBW)))) / 2 - (spanFactor * (int(ceil(getFFTLength()*(Display.span/Display.resBW)))) / 2)));
            int indexStopFreq = int(ceil(getFFTLength()*(Display.span/Display.resBW)) / 2 + (spanFactor * (int(ceil(getFFTLength()*(Display.span/Display.resBW)))) / 2));

            float startFreq = XAxisBuffer[indexStartFreq];
            float stopFreq = XAxisBuffer[indexStopFreq];
            //printf("indexStartFreq :%d",indexStartFreq );
            double *powerValues;
            // 假设其他变量已经正确定义和初始化
            // 首先计算表达式的结果，然后应用ceil，最后转换为整型
            int arraySize = static_cast<int>(ceil(spanFactor * ceil(getFFTLength() * (Display.span / Display.resBW))));

            // 使用这个整型作为数组的大小
            powerValues = new double[arraySize];


            //printf("arraySize: %d\n", arraySize);  // 打印 arraySize 的值
            int loopUpperLimit = ceil(spanFactor * int(ceil(getFFTLength()*(Display.span/Display.resBW))));
            //printf("Loop upper limit: %d\n", loopUpperLimit); // 打印循环上限的值

            // 打印 indexStartFreq 的值
            //printf("indexStartFreq: %d\n", indexStartFreq);



            for(int x = 0; x<loopUpperLimit; x++)
            {
                // Save channel power values in Watt
                powerValues[x] = pow(10, YAxisBuffer[indexStartFreq+x]/10)/1000;
                //printf("x: %d, YAxisBuffer[indexStartFreq+x]: %f", x, YAxisBuffer[indexStartFreq+x]);
            }
            double chPower = 0;
            // Calculate sum of power values to receive the channel power
            for(int x = 0; x<ceil(spanFactor * int(ceil(getFFTLength()*(Display.span/Display.resBW)))); x++)
            {
            ...
            char buffer1[160];
            sprintf(buffer1, "set object 2 rect from %f,%f to %f,%f behind fillcolor rgb 'red' fillstyle solid 0.5 border", startFreq, Display.refAmp-130, stopFreq, Display.refAmp);
        ...
        void Display_Markers()
        {
            for(int x = 0; x<3; x++)
            {
                if(Meas.marker[x] != 0)
                {
                    ...
                    char buffer[15];
                    snprintf(buffer, sizeof(buffer), "unset label %i", Meas.markerLabelID[x][0]);
                    GnuplotHandle.cmd(buffer);
                    snprintf(buffer, sizeof(buffer), "unset label %i", Meas.markerLabelID[x][1]);
                    GnuplotHandle.cmd(buffer);
                    //printf("Meas.marker[x]: %f\n(Display.centerFreq - Display.span/2):%f\n,int(ceil(getFFTLength()/2*(Display.span/Display.resBW))):%d\n ceil(getFFTLength()/2*(Display.span/Display.resBW)):%f \n getFFTLength()/2*(Display.span/Display.resBW):%f\n ", ceil(Meas.marker[x]),(Display.centerFreq - Display.span/2),int(ceil(getFFTLength()/2*(Display.span/Display.resBW))),ceil(getFFTLength()/2*(Display.span/Display.resBW)),getFFTLength()/2*(Display.span/Display.resBW));
                    // Set marker visual into plot
                    int XAxisIndex = ceil((Meas.marker[x] - (Display.centerFreq - Display.span/2))/Display.span*int(ceil(getFFTLength()*(Display.span/Display.resBW))));
                    char buffer1[80];
                    //set label 'origin' at 0,0 point lt 1 pt 2 ps 3 offset 1,-1
                    snprintf(buffer1, sizeof(buffer1), "set label %i 'T%i' at %f, %f front tc 'blue'", Meas.markerLabelID[x][0], (x + 1), XAxisBuffer[XAxisIndex], YAxisBuffer[XAxisIndex]);
                    GnuplotHandle.cmd(buffer1);
                    if(isnan(XAxisBuffer[XAxisIndex]) || isnan(YAxisBuffer[XAxisIndex]) || isinf(XAxisBuffer[XAxisIndex]) || isinf(YAxisBuffer[XAxisIndex])) {
                                    printf("Invalid data detected at index %d: X: %f, Y: %f\n", XAxisIndex, XAxisBuffer[XAxisIndex], YAxisBuffer[XAxisIndex]);
                                    continue;
                                }
                    // Display power value
                    char buffer2[90];
                    snprintf(buffer2, sizeof(buffer2), "set label %i 'Marker 1: %.2f dBm' at %.6f, %.6f", Meas.markerLabelID[x][1], YAxisBuffer[XAxisIndex], Meas.markerLabelPos[x][0], Meas.markerLabelPos[x][1]);
                    GnuplotHandle.cmd(buffer2);
                }
            }
        };
...
int main(int argc, char** argv)
{
    ...
    float_type centerFreq = 1.012e9;// (Carrier frequency?)
    float_type span = 45e6;//xdisplay span
    ...
    //Trace1.Change_CH_Measurement_Settings(5e6);
    Trace1.setRefAmp();
