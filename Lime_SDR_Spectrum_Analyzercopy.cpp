...
class Lime_SDR_mini_Rx
{
    ...
    public:
        ...
        int Setup_SDR(float_type centerFreq, float_type sampleRate, unsigned gain)
        {
            ...
            const char *config_file = "./true2400.ini";
...
class Trace
{
    public:
        ...
        void setRefAmp()
        {
            GnuplotHandle.set_yrange(Display.refAmp-112, Display.refAmp);
        };
    ...
    private:
        ...
        void CH_Measurement()
        {
            ...
            int arraySize = static_cast<int>(ceil(spanFactor * ceil(getFFTLength() / 2 * (Display.span / Display.resBW))));
            ...
            int loopUpperLimit = ceil(spanFactor * int(ceil(getFFTLength()/2*(Display.span/Display.resBW))));
        ...
        void Display_Markers()
        {
            for(int x = 0; x<3; x++)
            {
                if(Meas.marker[x] != 0)
                {
                    ...
                    int XAxisIndex = ceil((Meas.marker[x] - (Display.centerFreq - Display.span/2))/Display.span*int(ceil(getFFTLength()/2*(Display.span/Display.resBW))));
                    char buffer1[80];
...
int main(int argc, char** argv)
{
    ...
    float_type centerFreq = 2.4e9;// (Carrier frequency?)
    float_type span = 30e6;//x3display span
    float_type sample = 30e6;//bandwidth
    double gain = 50;
    double refAmp = 0;
    int FFTlength = 16384;
    int averaging = 0;
