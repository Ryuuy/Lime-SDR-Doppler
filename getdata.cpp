...
public:
    ...
    int Setup_SDR(float_type centerFreq, float_type sampleRate, unsigned gain)
    {
        ...
        const char* config_file = "./900mimo.ini";
...
int main(int argc, char** argv)
{
    ...
    float_type centerFreq = 900e6;   // Carrier frequency
    float_type span =  15e6;          // Frequency span
    float_type sample = 15e6;        // Bandwidth
    double gain = 30;
    double refAmp = 0;
    int FFTlength = 16384/32;
    int averaging = 0;
    unsigned nTrace = 1;
    ...
    // Create Trace object
    std::string folderName = "data/nohumananother2501Ghz";
    ...
    for (int i = 0; i < 2; ++i) {
        streamBuffers[i] = new int16_t[FFTlength * 2]; // 分配缓冲区
    }

    for (const auto& folder : folderNames) {
        EnsureFolderExists(folder); // 调用函数创建文件夹
    }
    ...
    for (int u = 0; u < 1; u++) {
        ...
        Trace1.Acquire_Sweep_Buffered_MultiChannel(framenumber, folderNames, FFTlength, streamBuffers, bufferSize, repeat, numChannels);  // Pass the required parameters
